...
void setup()  {
  ...
  for ( int i=0 ; i<num_W ; i++ )  {  // Populate grid with points
...
void draw()  {  
  background( 255 );
  for ( int i=0 ; i<num_W ; i++ )  { // Iterate through points and draw lines
    for ( int j=0 ; j<num_H ; j++ )  {
      strokeWeight(4);
      ...
      strokeWeight(1);
      if ( i > 0 )  {
        line( PTs.get( i*num_H + j).p_.x, PTs.get( i*num_H + j).p_.y,  // Connect the points
